...
\midrule
  ...
  \Y{\M<13->38438}  & \Y{\M<13->38136} &  \Y{\M<13->12} & \Y{\MM<13->{\bbash{ps -o pid,ppid,cpuid,cmd}}}
   ...
   \only<0|handout:2>{%
      % tikzmark is problematic due to nested tikz and me being lzy
      \draw[-Kite,T] ([xshift=-.75cm,yshift=-2.75cm]@2.north) to[out=270,in=180] ++(.65,-.125) node[below right,xshift=.5ex,align=left,yshift=.2\baselineskip,scale=.8] {Used for generic splitting magic\\(e.g., not done with \bbash{--roundrobin})};
   }
\end{tikzpicture}
...
\begin{layout-imageonly}
   \begin{minted}[morekeywords={[2]{string}}]{javascript}
...
\def\Q{\only<8->}
\begin{frame}[fragile,t]{Integrating JavaScript}
...
\begin{tikzpicture}[@O]
   ...
   \onslide<4->{
      \draw[T,-Kite] ([yshift=1pt,xshift=-1pt]roundrobin.6) to[out=80,in=260] ++(.33,.85) node[above right,xshift=-1.2ex,yshift=-.325mm] (@n) {Distributes \setbox0=\hbox{records}\kern.5\wd0\clap{\copy0}\clap{\raisebox{\baselineskip}{\scriptsize\only<5->{\T{-{}-recstart ''{}''}, \T{-{}-recend ''\textbackslash n''}}}}\kern.5\wd0~amongst all jobs};
      \node[below right,yshift=2mm,xshift=5mm,T] at(@n.south west) {No longer guarantees order.};
   ...
   \onslide<6->{\draw[T,-Kite] ([yshift=.25mm,xshift=.125mm]yarn@) to[out=0,in=180] ++(.65,-.5) node[right,yshift=-.33mm] {Runs \T{tsc \&\& node consumer.js}};}
   \onslide<7->{\node[above right,xshift=\beamer@leftmargin,yshift=1cm,font=\tiny\ttfamily,text width=14cm] (@) at(current page.south west) {\begin{tabular}{rrrl}
      {pid} & {ppid} & {cpuid} & {cmd} \\ % {lwp} & \rlap{\quad\ldots}
\midrule
 ...
 \M{\Q{258487}} & \Q{258484} & \Q{3} & \MM{node /home/lord-waddle/.nvm/versions/node/v16.15.1/bin/yarn --silent start-consumer} \\
  % TODO: note much more compact
 ...
 % TODO: note, compile process has already ended here, others would be there just the same (pipes buffer)
 \M{\Q{258605}} & \Q{258604} & \Q{ 4} & \MM{/home/lord-waddle/.nvm/versions/node/v16.15.1/bin/node consumer.js}\\
 \M{\Q{258618}} & \M{\Q{258480}} & \Q{11} & \Q{\K{\bbash{ps -o pid,ppid,cpuid,cmd}}}
   \end{tabular}};
   }
\end{tikzpicture}
\end{frame}
...
% TODO: custom replacement strings with rpl --group-by etc
