...
\begin{tikzpicture}[@O]
   ...
   \onslide<4->{\hlbehindcodeunder{lazy}{@lazy>}
      \draw[T,-Kite] (@lazy>_down) to[bend right] ++(.5,-.35) node[right] {Use 0 as initial value};
   ...
   \onslide<6->{\hlbehindcodeunder{cb2}{@cb2>}
      \draw[T,-Kite] (@cb1>_down) to[bend left] ++(.5,-.35) node[right] {Anonymous arrow style callback for 'closed' event};
   }
...
\begin{tikzpicture}[@O]
   ...
   \onslide<4->{
      \draw[T,-Kite] ([yshift=1pt,xshift=-1pt]roundrobin.6) to[out=80,in=260] ++(.66,.85) node[above right,xshift=-1.2ex,yshift=-.325mm] (@n) {Distributes \setbox0=\hbox{records}\kern.5\wd0\clap{\copy0}\clap{\raisebox{\baselineskip}{\scriptsize\only<5->{\T{-{}-recstart ''{}''}, \T{-{}-recend ''\textbackslash n''}}}}\kern.5\wd0~amongst all jobs};
...
\begin{frame}[fragile]{Data Encoding}
   \begin{itemize}[<+(1)->]
      \item By using pipes we can pass on any data (e.g. plain text or binary)
...
% TODO: mention unidirectional pipes
\end{frame}
\begin{frame}
