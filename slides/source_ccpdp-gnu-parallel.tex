...
% git://git.savannah.gnu.org/parallel.gi
\begin{document}
\section{Motivation}
...
\scope[shift={(current page.north east)},shift={(-4cm,-2cm)},codeouthl,xscale=2,K/.style={circle,fill,scale=1.25},scale=.5,every ndoe/.append style={transform shape}]
      ...
      {\only<17-19|handout:4>{\color{shadeA}}
         \node[K] (4) at (3,0) {};
      }
...
&ihl&	| java -jar accountant.jar&ihl&
\end{minted}
\end{onlyenv}
\begin{onlyenv}<10-|handout:2>
\begin{minted}{bash}
...
\midrule
  38279 & \M<13->38276 &  \Y{\M<13->3} & \Y{\MM<13->{\bbash{/bin/bash -c java -jar producer.jar 1000000 | parallel --pipe java -jar consumer.jar | java -jar a:ldots:}}}\kern-3pt\\
  \Y{\M<13->38280} & 3\Y{8279} &  \Y{6} & \Y{\K{\bbash{java -jar producer.jar 1000000}}} \\
  ...
  \Y{38344} & \Y{38281} & \Y{10} & \Y{\K{\bbash{perl -e if(sysread(STDIN,\$buf,1))\{open(\$fh }}}\textcolor{gray}{\,[\ldots]\!}\Y{\K{\} \bbash{/usr/bin/bash -c java -jar consumer.jar}}} \\
  \Y{\M<13->38345} & \Y{38281} &   \Y{\M<13->4} & \MM<13->{\Y{\K{\bbash{perl -e if(sysread(STDIN,\$buf,1))\{open(\$fh }}}\textcolor{gray}{\,[\ldots]\!}\Y{\K{\} \bbash{/usr/bin/bash -c java -jar consumer.jar}}}} \\
  \color{gray}\ldots & \color{gray}\ldots & \color{gray}\ldots & \color{gray}\ldots \\
  \Y{\M<13->38363} & \Y{38281} &  \Y{\M<13->10} & \MM<13->{\Y{\K{\bbash{perl -e if(sysread(STDIN,\$buf,1))\{open(\$fh }}}\textcolor{gray}{\,[\ldots]\!}\Y{\K{\} \bbash{/usr/bin/bash -c java -jar consumer.jar}}}} \\
  \Y{\M<13->38383}  & \Y{38281} &   \Y{\M<13->4} & \Y{\MM<8->{\bbash{perl /usr/bin/parallel --pipe java -jar consumer.jar}}} \\
  \Y{\M<13->38384}  & \Y{38344} &  \Y{2} & \Y{\K{\bbash{java -jar consumer.jar}}} \\
  \Y{\M<13->38438}  & \Y{\M<13->38136} &  \Y{\M<13->12} & \Y{\MM<8->{\bbash{ps -o pid,ppid,cpuid,cmd}}}
   \end{tabular}};}


   % below right for same position! so it does not jump
   \onslide<6-|handout:1>{\node[below right,font=\tiny\ttfamily,text width=14cm] at(@.north west) {\begin{tabular}{rrrl}
         {pid} & {ppid} & {cpuid} & {cmd} \\ % {lwp} & \rlap{\quad\ldots}
   \midrule
   25621 & \M<8->25618 &  \Q{\M<8->7} &\rlap{\Q{\MM<8->{\bbash{/bin/bash -c java -jar producer.jar 1000000 | java -jar consumer.jar | java -jar accountant.jar}}}}% align lap:
   \phantom{\bbash{/bin/bash -c java -jar producer.jar 1000000 | parallel --pipe java -jar consumer.jar | java -jar a:ldots:}\kern-3pt}\\
   \Q{\M<8->25622} & \Q{25621}  &  2 & \Q{\K{\bbash{java -jar producer.jar 1000000}}} \\
...
parallel -S $SERVER echo running on ::: $SERVER
   ...
   \begin{minted}[deletekeywords={transferfile,return}]{bash}
parallel -S $SERVER --return {}.out --transferfile\
   {} --cleanup cat {} ">"{}.out ::: input_file
   \end{minted}
%       // Shorthand
%       // parallel -S $SERVER --trc {}.out cat {} ">"{}.out ::: input_file
