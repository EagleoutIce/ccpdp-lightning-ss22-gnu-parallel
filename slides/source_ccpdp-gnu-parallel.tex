...
\midrule
 ...
 \Q{258487} & \Q{258484} & \Q{3} & \Q{\MM{node /home/lord-waddle/.nvm/versions/node/v16.15.1/bin/yarn --silent start-consumer}} \\
...
\only<0|handout:1>{%
   % tikzmark is problematic due to nested tikz and me being lzy
   \draw[-Kite,T] ([xshift=-.85cm,yshift=-1.65cm]@2.north) to[out=0,in=185] ++(.5,-.15) node[right,xshift=.25ex,align=left] {Much simpler (reuses workers for records)};
   \draw[-Kite,T] ([xshift=.45cm,yshift=-2.5cm]@2.north) to[bend left] ++(.5,.25) node[right,xshift=.25ex,align=left] {Compilation (\T{tsc}) already completed};
