...
\begin{tikzpicture}[@O]
   ...
   \onslide<17->{
      ...
      \draw[T,-Kite] (@value>_down) to[bend left] ++(-.5,-.35) node[left] {2};
   }
\end{tikzpicture}
\begin{minted}{java}
...
while (scanner.hasNextLine()) {
   ...
   System.out.printf("%s -%d%n", from, Integer.parseInt(s[VALUE]));/*\vspace{\medskipamount+1mm}*/
   String to = NAMES[Integer.parseInt(s[/*\Snode{to}*/TO/*\Snode{to@}*/])];
   System.out.printf("%s %d%n", to, Integer.parseInt(s[/*\Snode{value}*/VALUE/*\Snode{value@}*/]));
}
scanner.close();
\end{minted}
% TODO: markers for from to and value
\end{onlyenv}
...
\end{minted}
% TODO: expain sys.println and getOrDefault
\end{onlyenv}
\end{layout-imageonly}
\end{onlyenv}
\end{frame}
...
\begin{tikzpicture}[@O]
\scope[shift={(current page.north east)},shift={(-4cm,-2cm)},codeouthl,xscale=2,K/.style={circle,fill,scale=1.25},scale=.5,every ndoe/.append style={transform shape}]
      \only<2-6|handout:1>{{\only<3|handout:0>{\color{shadeA}}\node[K] (1) at (0,0) {};}
